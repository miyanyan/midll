...
using Elf32_Ehdr_ = Elf_Ehdr_template<uint32_t>;
using Elf64_Ehdr_ = Elf_Ehdr_template<uint64_t>;
...
using Elf32_Shdr_ = Elf_Shdr_template<uint32_t>;
using Elf64_Shdr_ = Elf_Shdr_template<uint64_t>;

template<class AddressOffsetT>
struct Elf_Sym_template;

template<>
struct Elf_Sym_template<uint32_t>
{
    using AddressOffsetT = uint32_t;
...
using Elf32_Sym_ = Elf_Sym_template<uint32_t>;
using Elf64_Sym_ = Elf_Sym_template<uint64_t>;

template<class AddressOffsetT>
class elf_info
{
    using header_t = midll::detail::Elf_Ehdr_template<AddressOffsetT>;
    using section_t = midll::detail::Elf_Shdr_template<AddressOffsetT>;
    using symbol_t = midll::detail::Elf_Sym_template<AddressOffsetT>;

    static constexpr uint32_t SHT_SYMTAB_ = 2;
    static constexpr uint32_t SHT_STRTAB_ = 3;
    static constexpr uint32_t SHT_DYNSYM_ = 11;
...
public:
    ...
    {
        const unsigned char magic_bytes[5] = {0x7f, 'E', 'L', 'F', sizeof(uint32_t) == sizeof(AddressOffsetT) ? 1 : 2};
...
private:
    ...
    static bool is_visible(const symbol_t& sym) noexcept
    {
        ...
        return (visibility == STV_DEFAULT_ || visibility == STV_PROTECTED_) && (sym.st_info >> 4) != STB_LOCAL_ &&
               !!sym.st_size;
    }
...
using elf_info32 = elf_info<uint32_t>;
using elf_info64 = elf_info<uint64_t>;
